...
& "$PSScriptRoot\kt_demo_stop.ps1" -RobotIp $config.RobotIp -RobotUser $config.RobotUser -RosbridgePort $config.RosbridgePort -MjpegPort $config.MjpegPort -RemoteWorkspace $config.RemoteWorkspace -VideoDevice $config.VideoDevice -LidarDevice $config.LidarDevice
