...
sleep 3 &&
tmux new-session -d -s mjpeg_bridge 'bash -lc "source ~/.bashrc >/dev/null 2>&1 || true; command -v ros2_setup >/dev/null 2>&1 && ros2_setup >/dev/null 2>&1 || true; source ~/tianracer_ros2_ws/install/setup.bash >/dev/null 2>&1 || true; python3 ~/tianracer_ros2_ws/src/kt_visual_lidar_follow/scripts/kt_mjpeg_bridge.py --port 8080 --fps 8 2>&1 | tee /tmp/mjpeg_bridge.log; sleep 3600"' &&
sleep 5 &&
